...
globalThis.startGenerationTest = (limited = false, smallInput = false) => {
  ...
  run = { pageId, requestId: crypto.randomUUID(), started: performance.now(), samples: [], storageEvents: [], visibility,
...
globalThis.generationTestState = () => ({ pageId, requestId: run?.requestId, done: !!run?.done,
  error: run?.error, samples: run?.samples.length || 0, final: run?.samples.at(-1), visibility: [...visibility], disposed: run?.disposed });
