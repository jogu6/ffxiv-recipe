...
export async function openOpfsSearchStore({ onActivity = () => {} } = {}) {
  ...
  const metrics = {
    storageBackend: 'opfs', storageReservationMode: 'opfs-file-size',
    storageRequestedBytes: 0, storageReadTransactions: 0, storageWriteTransactions: 0,
    storageReadMs: 0, storageWriteMs: 0, storageReadBytes: 0, storageWrittenBytes: 0,
    storageFlushMs: 0, storageFlushCount: 0,
  ...
  return {
    ...
    write(at, bytes) {
      ...
      if (metrics.storageWrittenBytes - flushedWrittenBytes >= FLUSH_BYTES) {
        ...
        metrics.storageFlushMs += performance.now() - flushStarted;
        metrics.storageFlushCount++;
