...
impl ParetoFrontBuilder {
    ...
    pub fn push_front(&mut self, front: crate::memory::FrontRef<'_>, addition: ParetoValue) {
        std::mem::swap(&mut self.result, &mut self.merge_buffer);
        let slice_b_begin = self.merge_buffer.len();
        front.reserve(&mut self.result, slice_b_begin + front.len());
        front.append_transformed(&mut self.merge_buffer, addition);
