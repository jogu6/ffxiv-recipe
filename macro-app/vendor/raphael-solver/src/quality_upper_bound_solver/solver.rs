...
pub type ParetoFront<'a> = FrontRef<'a>;
pub type SolvedStates<'alloc> = FxHashMap<ReducedState, ParetoFront<'alloc>>;
...
struct TemplateRecord<'alloc> {
    ...
    slots: Vec<Option<ParetoFront<'alloc>>>,
...
impl<'alloc> QualityUbSolver<'alloc> {
    ...
    fn lookup_slot(&self, state: &ReducedState) -> Option<ParetoFront<'alloc>> {
            ...
            for stellar_steady_hand in 0..3 {
                ...
                for cp in (min_solved_cp..=self.context.settings.max_cp()).step_by(2) {
                    ...
                    for (template_data, pareto_front) in solved_states {
                        ...
                        // Adjacent CP budgets frequently produce identical fronts. Share
                        // immutable values after an exact comparison, not a hash-only match.
                        let previous = index.checked_sub(1).and_then(|i| slots[i]);
                        let length_checked = if let Some(previous) = previous
                            && previous.equals_slice(&pareto_front) {
                            previous
                        } else {
                            allocator.save(&pareto_front)?
                        };
            ...
            if let Some((new_state, progress, quality)) = state.use_action(
                ...
                if !new_state.is_final(self.context.durability_cost) {
                    if let Some(pareto_front) = self.lookup_slot(&new_state) {
                        pf_builder.push_front(pareto_front, action_value);
                    } else {
...
impl<'main, 'alloc> QualityUbSolverShard<'main, 'alloc> {
    ...
    fn lookup_shared(&self, state: &ReducedState) -> Option<ParetoFront<'alloc>> {
        ...
        for action in FULL_SEARCH_ACTIONS {
            if let Some((child_state, progress, quality)) = state.use_action(
                ...
                if !child_state.is_final(self.context.durability_cost) {
                    ...
                    pareto_front_builder.push_front(child_pareto_front, action_value);
