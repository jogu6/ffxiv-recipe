function Download-Icons {
    ...
    $logFolder = Join-Path $pipelineRoot "logs"
    ...
    foreach ($item in $items) {
        ...
        $groupFolder = Join-Path $outputFolder $fileName.Substring(0, 3)
        if (!(Test-Path $groupFolder)) { New-Item -ItemType Directory -Path $groupFolder | Out-Null }
        $outputPath = Join-Path $groupFolder $fileName
...
$itemJson = Join-Path $repositoryRoot "site\data\Item.json"
$pictureFolder = Join-Path $repositoryRoot "site\assets\item-icons"
